...
button_to_chat = InlineKeyboardButton('Пообщаться с нейросетью', callback_data='chat')

main_menu_kb = InlineKeyboardMarkup(row_width=1).add(button_to_voice_input).add(button_to_interview).add(button_to_api).add(button_to_schedule).add(button_to_chat)
